    ...
    public partial class Crashes : AppCenterService
    {
        ...
        private void InstanceTrackError(System.Exception exception, IDictionary<string, string> properties, ErrorAttachmentLog[] attachments)
        {
            lock (_serviceLock)
            {
                ...
                var log = new HandledErrorLog(exception: exceptionAndBinaries.Exception, binaries: exceptionAndBinaries.Binaries, properties: properties, id: errorId, device: null, userId: UserIdContext.Instance.UserId, dataResidencyRegion: AppCenter.PlatformGetDataResidensyRegion());
        ...
        private Task SendErrorAttachmentsAsync(Guid errorId, IEnumerable<ErrorAttachmentLog> attachments)
        {
            ...
            foreach (var attachment in attachments)
            {
                if (attachment != null)
                {
                    ...
                    attachment.DataResidencyRegion = attachment.DataResidencyRegion ?? AppCenter.PlatformGetDataResidensyRegion();
