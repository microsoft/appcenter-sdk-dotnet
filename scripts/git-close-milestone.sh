...
for ISSUE_NUMBER in $OPENED_ISSUES; do
    POST_COMMENT_URL="$GITHUB_API_URL/issues/$ISSUE_NUMBER/comments"
...
EOF
    ...
    curl -fsS -o /dev/null -H "Authorization: token $GITHUB_ACCESS_TOKEN" -X POST $POST_COMMENT_URL -d "$body" || exit

    CLOSE_ISSUE_URL="$GITHUB_API_URL/issues/$ISSUE_NUMBER"
    echo "Closing $ISSUE_NUMBER issue..."
    echo $CLOSE_ISSUE_URL
    curl -fsS -o /dev/null -H "Authorization: token $GITHUB_ACCESS_TOKEN" -X PATCH $CLOSE_ISSUE_URL -d "{ \"state\": \"closed\" }" || exit
done

# Close milestone
CLOSE_MILESTONE_URL="$GITHUB_API_URL/milestones/$MILESTONE_NUMBER"
echo "Closing $MILESTONE_NUMBER milestone..."
echo $CLOSE_MILESTONE_URL
curl -fsS -o /dev/null -H "Authorization: token $GITHUB_ACCESS_TOKEN" -X PATCH $CLOSE_MILESTONE_URL -d "{ \"state\": \"closed\" }" || exit
