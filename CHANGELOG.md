...
* **[Feature]** Improve using TLS protocol for application with target framework version `4.7.1` or higher. For applications with the target framework version lower than `4.7.1` TLS connection still is configured via `ServicePointManager`.
